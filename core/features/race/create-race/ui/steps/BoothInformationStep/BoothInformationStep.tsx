...
export const BoothInformationStep = () => {
    ...
    return (
        <CreateRaceStepLayout step={2} title="THÔNG TIN TRẠM">
            <div className="overflow-visible rounded-xl border border-[#e5e5e5] bg-white shadow-sm">
                <div className="max-h-[calc(100svh-360px)] overflow-x-auto overflow-y-visible">
                    <div className="min-w-[1180px]">
                        <div className="sticky top-0 z-10 grid grid-cols-[1fr_.8fr_1.2fr_1.2fr_96px] gap-4 border-b border-[#eeeeee] bg-[#fbfbfb] px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#737373]"><span>Tên trạm</span><span>Địa điểm</span><span>Quản trạm</span><span>Mô tả trạm</span><span /></div>
                        {rows.map((row) => {
                            const descriptionText = row.description.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();

                            return (
                                <div key={row.id} className="grid grid-cols-[1fr_.8fr_1.2fr_1.2fr_96px] items-start gap-4 border-b border-[#f2f2f2] px-5 py-3 text-sm text-[#525252] transition-colors hover:bg-[#fcfcfc]">
                                    <Input
                        ...
                        <div key={`new-station-${rows.length}`} className="grid grid-cols-[1fr_.8fr_1.2fr_1.2fr_96px] items-start gap-4 px-5 py-3">
