import { Button, Drawer, Input, RichTextEditor, Switch } from '@/core/shared'
...
export const BoothInformationStep = () => {
    ...
    return (
        <CreateRaceStepLayout step={2} title="THÔNG TIN TRẠM">
            <div className="min-w-0 overflow-hidden rounded-xl border border-[#e5e5e5] bg-white shadow-sm">
                <div className="max-h-[calc(100svh-360px)] min-w-0 overflow-y-auto overflow-x-hidden">
                    <div className="w-full min-w-0">
                        <div className="sticky top-0 z-10 hidden grid-cols-[minmax(0,1fr)_minmax(0,.85fr)_minmax(0,1.2fr)_minmax(0,1.1fr)_80px_44px] gap-4 border-b border-[#eeeeee] bg-[#fbfbfb] px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#737373] md:grid"><span>Tên trạm</span><span>Địa điểm</span><span>Quản trạm</span><span>Luật trạm</span><span className="text-center">Trạm ẩn</span><span /></div>
                        {rows.map((row) => {
                            const descriptionText = row.description.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();

                            return (
                                <div key={row.id} className="grid min-w-0 grid-cols-1 items-start gap-3 border-b border-[#f2f2f2] px-5 py-3 text-sm text-[#525252] transition-colors hover:bg-[#fcfcfc] md:grid-cols-[minmax(0,1fr)_minmax(0,.85fr)_minmax(0,1.2fr)_minmax(0,1.1fr)_80px_44px] md:gap-4">
                                    <Input
                                    ...
                                    <div className="flex min-h-10 items-center justify-between rounded-lg border border-[#eeeeee] bg-[#fcfcfc] px-3 md:justify-center md:border-0 md:bg-transparent md:px-0">
                                        <span className="text-sm text-[#737373] md:hidden">Trạm ẩn</span>
                                        <Switch
                                            checked={row.isHidden}
                                            onChange={(isHidden) => updateHiddenStatus(row.id, isHidden)}
                                        />
                                    </div>
                        ...
                        <div key={`new-station-${rows.length}`} className="grid min-w-0 grid-cols-1 items-start gap-3 px-5 py-3 md:grid-cols-[minmax(0,1fr)_minmax(0,.85fr)_minmax(0,1.2fr)_minmax(0,1.1fr)_80px_44px] md:gap-4">
                            ...
                            <OrganizerSearchBox type="single" onChange={(organizers) => createStation({ managers: organizers })} />
                            ...
                            <div className="flex min-h-10 items-center justify-between rounded-lg border border-dashed border-[#e2e2e2] bg-white px-3 md:justify-center md:border-0 md:px-0">
                                <span className="text-sm text-[#9ca3af] md:hidden">Trạm ẩn</span>
                                <Switch checked={false} onChange={createHiddenStation} />
                            </div>
            ...
            <Drawer open={Boolean(selectedStation)} panelClassName="!max-w-[760px]" title={selectedStation ? `Luật trạm: ${selectedStation.name || 'Trạm mới'}` : 'Luật trạm'} onClose={closeDetails} footer={<><Button variant="secondary" onClick={closeDetails}>Hủy</Button><Button onClick={closeDetails}>Lưu</Button></>}>
