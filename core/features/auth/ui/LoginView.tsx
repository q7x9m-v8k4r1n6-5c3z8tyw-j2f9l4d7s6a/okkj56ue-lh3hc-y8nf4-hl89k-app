import { ArrowRightIcon, LockIcon, UserIcon } from '@/core/assets/icons'
import { waterMarkMove } from '@/core/assets'
import { Button, Input } from '@/core/shared'
import loginBackground from './assets/login-background.jpg'
import { useLogin } from './hooks/useLogin'

/**
 * Renders the complete login workflow owned by the auth feature.
 *
 * Pages only mount this public view and do not provide feature logic, state,
 * assets, or entity data.
 */
export const LoginView = () => {
  const {
    fieldErrors,
    globalError,
    handlePasswordChange,
    handleStandardLogin,
    handleUsernameChange,
    isLoading,
    isSubmitDisabled,
    lockoutSeconds,  
    password,
    username,
  } = useLogin()

  return (
    <main
      className="flex min-h-screen items-center justify-center bg-cover bg-center bg-no-repeat px-4"
      style={{ backgroundImage: `url(${loginBackground})` }}
    >
      <section
        className="w-full max-w-md rounded-[20px] bg-white p-8 shadow-2xl"
        aria-labelledby="login-heading"
      >
        <header className="mb-8 flex flex-col items-center">
          <div className="flex h-[72px] w-[72px] items-center justify-center rounded-2xl bg-[#de3336] shadow-sm">
            <img
              src={waterMarkMove}
              alt="Move"
              className="h-10 w-10 object-contain"
            />
          </div>
          <h1
            id="login-heading"
            className="mt-4 text-3xl font-extrabold tracking-tight text-[#1a1c1c]"
          >
            Đăng nhập
          </h1>
        </header>

        <form onSubmit={handleStandardLogin} className="space-y-5">
          <Input
            label="Tên đăng nhập"
            value={username}
            onChange={handleUsernameChange}
            leadingIcon={<UserIcon className="h-4 w-4" />}
            disabled={isSubmitDisabled}
            requiredMark
            error={fieldErrors.username}
            placeholder="Điền tên đăng nhập vào đây"
          />

          <Input
            label="Mật khẩu"
            type="password"
            value={password}
            onChange={handlePasswordChange}
            placeholder="Điền mật khẩu vào đây"
            leadingIcon={<LockIcon className="h-4 w-4" />}
            disabled={isSubmitDisabled}
            requiredMark
            error={fieldErrors.password}
          />

          <p
            className={`rounded-lg text-xs font-medium text-red-600 ${
              globalError ? 'opacity-100' : 'opacity-0'
            }`}
            aria-live="polite"
          >
            {globalError || <span className="invisible">&nbsp;</span>}
          </p>

          <Button
            type="submit"
            size="md"
            variant="primary"
            className="w-full text-base"
            trailingIcon={!lockoutSeconds ? <ArrowRightIcon className="h-5 w-5" /> : undefined}
            disabled={isSubmitDisabled}
          >
            {lockoutSeconds > 0 
              ? `còn ${lockoutSeconds} giây` 
              : isLoading 
                ? 'Đang xử lý...' 
                : 'Đăng nhập'}
          </Button>
        </form>

        <div className="mt-8">
          <div className="relative">
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-[#e5e5e5]" />
            </div>
            <div className="relative flex justify-center text-sm">
              <span className="bg-white px-4 font-bold text-[#737373]">
                Nếu bạn là Organizer
              </span>
            </div>
          </div>

          <div className="mt-6 flex justify-center">
            <div
              id="googleSignInBtn"
              className="flex w-full justify-center overflow-hidden rounded-lg"
            />
          </div>
        </div>
      </section>
    </main>
  )
}