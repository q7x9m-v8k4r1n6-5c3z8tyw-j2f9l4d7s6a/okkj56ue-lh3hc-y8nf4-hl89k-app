...
export const RaceFormPage = ({ mode }: RaceFormPageProps) => {
  ...
  const existingRow = useMemo(() => mode === 'edit' ? getRaceRecord(raceId) : null, [mode, raceId])
  ...
  const [startAt, setStartAt] = useState(existingRow?.startAt ?? '')
  const [endAt, setEndAt] = useState(existingRow?.endAt ?? '')
  const [location, setLocation] = useState(existingRow?.location ?? '')
  const [imageName, setImageName] = useState(existingRow?.imageName ?? defaults.imageName)
  const [stations, setStations] = useState<MoveRaceStation[]>(() => cloneStationRows(existingRow?.stations ?? defaults.stations))
  const [teams, setTeams] = useState<MoveNameMissionRow[]>(() => cloneSimpleRows(existingRow?.teams ?? defaults.teams))
  const [organizers, setOrganizers] = useState<MoveNameMissionRow[]>(() => cloneSimpleRows(existingRow?.organizers ?? defaults.organizers))
  const [settingsRows, setSettingsRows] = useState<MoveNameMissionRow[]>(() => cloneSimpleRows(existingRow?.settingsRows ?? defaults.settingsRows))
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const startDate = new Date(startAt)
    const endDate = new Date(endAt)
    const status = mode === 'edit' && existingRow && existingRow.startAt === startAt && existingRow.endAt === endAt
      ? existingRow.status
      : resolveRaceStatus(startAt, endAt)

    const savedRow = saveRaceRecord({
      id: existingRow?.id ?? 0,
      name: name.trim(),
      location: location.trim(),
      startAt: startAt.trim(),
      endAt: endAt.trim(),
      startText: formatDateTimeForStorage(startAt),
      endText: formatDateTimeForStorage(endAt),
      members: teams.length,
      status,
      duration: Math.max(15, Math.round((endDate.getTime() - startDate.getTime()) / 60000)),
      imageName,
      stations,
      teams,
      organizers,
    ...
    if (!savedRow) {
      setError('Không thể lưu dữ liệu trận đấu trên trình duyệt hiện tại.')
      return
    }
