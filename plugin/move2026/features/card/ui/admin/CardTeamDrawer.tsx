...
type Props = {
  ...
  onSavePrice: (cardId: string, price: number) => Promise<void>
  onAssign: (input: { cardId: string; teamId: string; reason: string }) => Promise<void>
  onDelete: (input: { cardId: string; teamId: string; cardInstanceId: string; reason: string }) => Promise<void>
  raceId: string
}

const statusLabel: Record<CardTeam['status'], string> = {
  pending_purchase: 'Đang đồng bộ mua card',
  received: 'Chưa sử dụng',
  used: 'Đã sử dụng',
  deleted: 'Xóa',
}

export const CardTeamDrawer = ({ card, editable, open, pending, shopOpen, onClose, onSaveConfig, onSavePrice, onAssign, onDelete, raceId }: Props) => {
  ...
  const savePrice = async () => {
    ...
    if (!Number.isInteger(normalizedPrice) || normalizedPrice < 1) return
    try {
      await onSavePrice(card.cardId, normalizedPrice)
      toast({ title: 'Đã lưu giá Data Patch' })
    } catch (error) {
      toast({ title: 'Không thể lưu giá', description: error instanceof Error ? error.message : 'Vui lòng thử lại.', variant: 'danger' })
    }
  }
  const assign = async () => {
    if (!draft?.teamId || !draft.teamName) return
    await onAssign({ cardId: card.cardId, teamId: draft.teamId, reason: draft.reason })
    setDraft(null)
  }
      ...
      <Drawer open={open} title={`${card.cardName} · Danh sách team`} panelClassName="!max-w-[920px]" onClose={onClose}>
        <div className="space-y-6">
          <div className="rounded-xl border border-[#eeeeee] bg-[#fffafa] p-4">
            <div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-[#1a1c1c]">Cấu hình card</h3><p className="mt-1 text-xs text-[#737373]">Các input được định nghĩa bởi plugin. Trap dùng số điểm bị trừ.</p></div>{editable ? <Button size="sm" disabled={pending || shopOpen} leadingIcon={<SaveIcon className="size-4" />} onClick={() => void saveConfig()}>Lưu cấu hình</Button> : null}</div>
            {shopOpen && editable ? <p className="mb-3 rounded-lg bg-[#fff3f3] px-3 py-2 text-xs text-[#9f2427]">Đóng cửa hàng trước khi thay đổi giá hoặc cấu hình.</p> : null}
